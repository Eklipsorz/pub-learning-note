...
- webpack 會以src目錄下的資源來做處理，若將靜態資源放置在src/assets/目錄下，會按照webpack來做轉換和處理，比如
	- 圖片會壓縮
	- CSS會被進行轉譯、優化、進行後製
- 若將靜態資源放置在public下，並不會被webpack所處理，會被webpack複製該目錄來保持原檔案
- 靜態資源存放位置策略：
...
#🧠 webpack：若將靜態資源放置在src/assets目錄下的話，會是？ ->->-> `會按照webpack來做轉換和處理，比如- 圖片會壓縮 - CSS會被進行轉譯、優化、進行後製`
<!--SR:!2022-08-11,3,250-->

#🧠 webpack：若將靜態資源放置在public目錄下的話，會是？ ->->-> `若將靜態資源放置在public下，並不會被webpack所處理，會被webpack複製該目錄來保持原檔案`
<!--SR:!2022-08-11,3,250-->
