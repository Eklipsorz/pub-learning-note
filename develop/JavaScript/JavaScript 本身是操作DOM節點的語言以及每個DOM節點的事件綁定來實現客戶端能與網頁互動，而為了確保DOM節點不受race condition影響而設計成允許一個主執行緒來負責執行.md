...
### JavaScript 的執行緒
JavaScript 本身是操作DOM節點的語言以及每個DOM節點的事件綁定來實現客戶端能與網頁互動，而為了盡可能減緩DOM節點的race condition影響而設計成允許一個主執行緒來負責執行，但仍有可能會受到多個任務輪流排隊影響所造成的race conditon。

[[race condition 是指多個程式模組以不受控制的執行順序來彼此競爭最後結果，使結果不如開發者所預期的結果]]

身為單執行緒的JavaScript 能夠完全避免同個DOM節點上的race condition嗎？ 
Ans： 並不能，若是多個任務想存取同個DOM節點，那麼在單執行緒上只會是任務排隊競爭存取那DOM節點，最後導致最後結果不如預期
...
#🧠 JavaScript 本身是單執行緒嗎？為什麼？->->-> `是的，而為了盡可能減緩DOM節點的race condition影響而設計成允許一個主執行緒來負責執行`

#🧠 身為單執行緒的JavaScript 能夠完全避免同個DOM節點上的race condition嗎？ ->->-> `並不能，若是多個任務想存取同個DOM節點，那麼在單執行緒上只會是任務排隊競爭存取那DOM節點，最後導致最後結果不如預期`
