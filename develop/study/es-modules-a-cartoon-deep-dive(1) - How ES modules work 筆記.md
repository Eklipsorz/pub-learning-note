...
#🧠 ES module 以建構來舉例非同步的任務如何做？ ->->-> `瀏覽器會生成多個thread來同時向模組所在的主機發送請求索要對應模組，不同模組下的任務可以不必相互等待不同模組的任務，但對於同個模組的任務：下載->解析->建立關係圖會是同步的`
...
#🧠 ES module 以執行模組內容並確定模組實例的輸出內容來舉例非同步的任務如何做(提示，記得要等待) ->->-> `在做evaluation之前，必須等待所有模組的實例都完成才能做。不同模組的任務可以不必等待不同模組的任務，會依據著模組依賴圖來對不需要模組或者載入已經完成evaluation的模組進行evaluation，不同模組的實例化任務並不會等待其他模組的evaluation執行任務`
