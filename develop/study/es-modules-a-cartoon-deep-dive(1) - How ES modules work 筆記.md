...
#🧠 ES module 的載入： 確定模組要做之前，等待什麼->->-> `等待實例化完成`
...
#🧠 ES module 非同步載入是指哪方面的非同步載入？以模組實例化和它之前的建構來舉例(提示，記得要等待)->->-> `在做實例化之前，必須等待所有模組的建構都完成，才能做。不同模組的任務可以不必等待不同模組的任務，會依據著模組依賴圖來對不需要載入模組或者載入已經完成實例化的模組進行記憶體分配，不同模組的實例化任務並不會等待其他模組的實例化任務`


#🧠 ES module 非同步載入是指哪方面的非同步載入？以執行模組內容並確定模組實例的輸出內容來舉例(提示，記得要等待) ->->-> ``在做實例化之前，必須等待所有模組的建構都完成，才能做。不同模組的任務可以不必等待不同模組的任務，會依據著模組依賴圖來對不需要載入模組或者載入已經完成實例化的模組進行記憶體分配，不同模組的實例化任務並不會等待其他模組的實例化任務，當完成記憶體分配便會執行模組內容來決定每個記憶體區塊所存的輸出內容。`
