...
-   建構階段：主要會去解讀模組所在的主機、並且向該主機索要模組、然後將模組解析成module record、接著再另外依據import/export來建立模組依賴關係圖
-   模組實例化：遍歷模組依賴關係圖來找到不需要載入模組或者載入已經完成實例化的模組進行記憶體分配，其記憶體會是存放模組輸出的內容，會是以識別字對應著特定記憶體區塊，其初始值會按照函式宣告/var/const宣告規則來給定、對每個模組的import/export都能指向對應特定模組的識別字所指向的記憶體區塊
...
#🧠 ES module 的載入： 建構階段中會做什麼 ->->-> `主要會去解讀模組所在的主機、並且向該主機索要模組、然後將模組解析成module record、接著再另外依據import/export來建立模組依賴關係圖`
...
#🧠 ES module的載入： 執行模組內容並確定模組實例的輸出內容是會做什麼？ ->->-> `當做完實例化之後，就執行對應代碼並根據識別字來更新對應記憶體區塊存放的內容`
...
#🧠 ES module 是非同步載入嗎？還是同步載入？->->-> `非同步載入`

#🧠 ES module 為什麼得必須以非同步載入？ ->->-> `為了讓ES module加入前端開發上，但同時也要保證模組載入不會因為網路速度而影響客戶端的使用體驗，而採取非同步來進行`

#🧠 ES module 非同步載入是指哪方面的非同步載入？ 具體如何做->->-> `將ES module的載入工作分成三個步驟：建構階段、模組實例化、執行模組內容並確定，並讓三個步驟按照不同的模組進行非同步的載入，但對於同個模組下的階段，可能會是同步處理`

#🧠 ES module 非同步載入是指哪方面的非同步載入？以建構來舉例 ->->-> `瀏覽器會生成多個thread來同時向模組所在的主機發送請求索要對應模組，不同模組下的任務可以不必相互等待不同模組的任務，但對於同個模組的任務：下載->解析->建立關係圖會是同步的`

#🧠 ES module 非同步載入是指哪方面的非同步載入？以模組實例化和它之前的建構來舉例 ->->-> `不同模組的任務可以不必等待不同模組的任務，會依據著模組依賴圖來對不需要載入模組或者載入已經完成實例化的模組進行記憶體分配，不同模組的實例化任務並不會等待其他模組的實例化任務`


#🧠 ES module 非同步載入是指哪方面的非同步載入？以執行模組內容並確定模組實例的輸出內容來舉例 ->->-> `不同模組的任務可以不必等待不同模組的任務，會依據著模組依賴圖來對不需要載入模組或者載入已經完成實例化的模組進行記憶體分配，不同模組的實例化任務並不會等待其他模組的實例化任務，當完成記憶體分配便會執行模組內容來決定每個記憶體區塊所存的輸出內容，該任務的非同步性質會和實例化一樣`

#🧠 ES Module 標準是什麼？ ->->-> `ES Module 標準是說程式該如何解析ES模組成模組紀錄、如何實例化、確定值`
...
#🧠 瀏覽器的loader是做什麼？ ->->-> `負責載入各種檔案，並從而根據檔案種類來進行處理以及對於模組的載入工作`


#🧠 ES Module 標準沒制定一個標準來定義在一開始的模組載入中，如何獲取檔案/ES模組，有可能會對ES模組產生影響，請以瀏覽器的loader來說明 ->->-> `預設會是以HTML spec，直接解析模組所在的主機位置、向主機索要對應模組，並載入至本地端`
