...
最後根據import/export以圖狀資料結構來產生模組依賴關係圖
...
多個模組索要模組代表有N個任務會同時想要獲取模組，若執行緒數量和實際核心數夠讓每個任務執行的話，每個任務將會以執行緒同時索取模組，但若模組是相同的話，將會有N個相同模組，然而，實際上也只需要一個，所以這對於瀏覽器來說，由於多做載入而使效能衰減
...
使用module map＋鎖的機制，讓只有一個任務能夠負責特定模組檔案的索要，並在module map以模組所在的URL作為索引來標記fetching來告知其他任務每次做之前都會從module map檢查要找的模組是否還有沒有人找：
	...
	- 若找不到，任務就當那個第一個要載入特定模組的任務，並標記fetching
...
#🧠 ES module：如何避免多個不同模組索要相同模組而做的重複性索要？ ->->-> `使用module map＋鎖的機制，讓只有一個任務能夠負責特定模組檔案的索要，並在module map以模組所在的URL作為索引來標記fetching來告知其他任務每次做之前都會從module map檢查要找的模組是否還有沒有人找： - 若對應模組狀態顯示fetching，任務就找下一個想要載入的模組 - 若找不到，任務就當那個第一個要載入特定模組的任務，並標記fetching`
