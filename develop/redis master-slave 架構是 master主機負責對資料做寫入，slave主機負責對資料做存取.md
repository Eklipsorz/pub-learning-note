...
#🧠  redis master-slave 架構如何實現讀寫分離？->->-> `master主機來主要負責控管資料上變更和存取，而slave則是接收從master主機所儲存的資料進行同步，從而透過這些主機來共同持有同一個redis資料庫，並且只賦予寫權限給master主機，讀權限則賦予master主機和slave主機`
...
#🧠 redis master-slave 架構缺點: (提示故障時可用性和寫入)->->-> `master主機故障時，master主機所建構的結構將會因為無法共享資料而失效，需要人工修復、寫入時只有一個master主機，所以寫入效能會有所限制`
<!--SR:!2022-06-08,2,230-->
