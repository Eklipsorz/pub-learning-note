...
#🧠 JavaScript  驗證字串內容原本是否為數字字串的方法，考慮點是什麼(提示：數字字串是什麼？ 數字-> 字串，會不會存在著誤判?)->->-> `確定數字字串會是純粹的數字，還是也可以夾雜空格或者0，比如" 1234" 或者 "01234"、字串內容若出現可以被誤判成數字的非數字內容、字串內容是否可經由轉換來比對為數字`
