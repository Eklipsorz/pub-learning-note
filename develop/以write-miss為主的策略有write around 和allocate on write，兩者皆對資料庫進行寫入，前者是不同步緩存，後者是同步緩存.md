...
重點：
- allocate on write：如同字面上的意思，當要進行寫入時就分配記憶體區塊來進行寫入。
	- 運用在client、cache、db上的話，就是客戶端無法在緩存找到對應資料來寫入的話，緩存系統就從資料庫獲取對應資料並複製資料分配至緩存上，接著客戶端就從緩存中修改資料。
