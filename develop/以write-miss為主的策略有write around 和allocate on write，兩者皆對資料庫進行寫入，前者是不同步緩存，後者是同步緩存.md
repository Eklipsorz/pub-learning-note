...
- 這策略是當使用者未能在特定空間A中找到想要修改的資料，就直接寫入後備儲存空間B，策略會應用在客戶端場景、CPU場景：
...
#🧠 write-no-allocate 策略原理是什麼？ ->->-> `這策略是當使用者未能在特定空間A中找到想要修改的資料，就直接寫入後備儲存空間B`

#🧠 在有客戶端、緩存、資料庫的場景下：write-no-allocate 策略原理是什麼？ ->->-> `這策略是當使用者未能在特定空間A中找到想要修改的資料，就直接寫入後備儲存空間B，策略會應用在客戶端場景、CPU場景： - 客戶端場景：使用者會是客戶端，特定空間A會是緩存，儲存空間B為處於硬碟的資料庫`

#🧠 在有CPU、緩存、記憶體的場景下：write-no-allocate 策略原理是什麼？->->-> `write-no-allocate / No-write-allocate / write around，發生在write-miss才有的事件處理。CPU場景：使用者會是CPU，特定空間A會是離CPU最近的緩存空間，儲存空間B是離CPU較遠的記憶體`

#🧠 在有客戶端、緩存、資料庫的場景下： write around的單方面場景是什麼？(提示讀寫效率好嗎) ->->-> `單方面使用的話，會適合讀寫較少的場境，尤其是在寫較少的場景`
<!--SR:!2022-09-05,44,210-->
...
#🧠 allocate on write vs. write-no-allocate 之間差異為何？以客戶端為案例來說明 ->->-> `1. 寫入資料方式：前者會寫入資料至主要空間和後備空間；後者就只是寫入資料至後備空間 2. 要寫入的資料何時能夠儲存在緩存：前者只要發生write-miss就會同步；後者是要發生read-miss才有機會`
