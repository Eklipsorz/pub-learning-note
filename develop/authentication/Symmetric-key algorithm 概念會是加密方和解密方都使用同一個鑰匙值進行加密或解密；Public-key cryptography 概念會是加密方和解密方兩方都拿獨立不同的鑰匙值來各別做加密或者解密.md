...
- 數位簽章具體有簽名、驗證這兩個流程，簽名方和驗證方都擁有相同的hashing algorithm、相同的secret
	- 簽名方簽名流程：
		- 傳遞者使用特定hash secret、hash algorithm來對內容製作雜湊值，並利用公鑰加密雜湊值
		- 江平
	- 
	- 添加會使用hashing algorithm 、secret、特定資料內容來生成簽署值，在使用私鑰加密簽署值，添加加密後簽署值和憑證資料至原資料上
	- 驗證方驗證簽名：當接收到添加憑證的資料時，會分別使用公鑰解密出簽署值A和使用(與簽名方相同)的hashing algorithm、secret、接收到資料產生簽署值B，並比對簽署值A和簽署值B是否一樣，若一樣就沒被篡改；若不一樣就代表被篡改。
		- 不一樣的原因：資料內容篡改、惡意使用者假冒傳遞方
		- 一樣的原因：資料內容未被篡改、傳遞方是使用合法的公鑰
		- 使用加密/解密這方式可以確保在傳遞過程讓合法驗證方來接收正確的內容，不讓接到正確的內容
...
- 數位簽章主要解決以下問題：
	- 傳遞內容是否遭他人惡意修改過
	- 確定傳遞方身份未遭受他人冒名頂替
-  傳遞內容是否遭他人惡意修改過的解法：傳遞者先將內容製作簽署值並夾雜傳遞封包，然後傳遞至接收方，接收方在對內容製作另一個簽署值比對夾雜的簽署值是否一樣
...
#🧠 數位簽章具體有簽名、驗證這兩個流程，若驗證部分會是不一樣的話，可能性會是什麼？ ->->-> `資料內容篡改、惡意使用者假冒傳遞方`

#🧠 數位簽章具體有簽名、驗證這兩個流程，若驗證部分會是一樣的話，可能性會是什麼？->->-> `資料內容未被篡改、傳遞方是使用合法的公鑰`
