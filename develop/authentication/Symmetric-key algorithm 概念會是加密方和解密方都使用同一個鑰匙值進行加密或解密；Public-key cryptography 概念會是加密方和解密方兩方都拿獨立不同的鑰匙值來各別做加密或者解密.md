...
- 數位簽章具體有簽名、驗證這兩個流程，簽名方和驗證方都擁有相同的hashing algorithm、相同的secret
	...
	- 驗證方驗證簽名：當接收到添加憑證的資料時，會分別使用公鑰解密出簽署值A和使用(與簽名方相同)的hashing algorithm、secret、接收到資料產生簽署值B，並比對簽署值A和簽署值B是否一樣，若一樣就沒被篡改；若不一樣就代表被篡改。
		- 不一樣的原因：資料內容篡改、加密方使用錯誤的密鑰(惡意加密方)
		- 一樣的原因：資料內容未被篡改、加密方使用正確的密鑰加密
		- 使用這方式來驗證，可以更加確保簽章會是原簽發者
...
#🧠 數位簽章具體有簽名、驗證這兩個流程，若驗證部分會是不一樣的話，可能性會是什麼？ ->->-> `資料內容篡改、加密方使用錯誤的密鑰(惡意加密方)`

#🧠 數位簽章具體有簽名、驗證這兩個流程，若驗證部分會是不一樣的話，可能性會是資料內容篡改、加密方使用錯誤的密鑰(惡意加密方)，那這樣能確保什麼？？ ->->-> `可以更加確保簽章會是原簽發者`

#🧠 數位簽章具體有簽名、驗證這兩個流程，流程添加非對稱加解密，是要確保什麼？ ->->-> `可以更加確保簽章會是原簽發者`

#🧠 數位簽章具體有簽名、驗證這兩個流程，流程添加對稱加解密，會比非對稱加解密來得好嗎？ 為啥？->->-> `不好，不好確定簽章是由原簽發者簽發，原因為若鑰匙公開，等同`
