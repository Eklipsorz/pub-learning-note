...
- 每一個declaration的權重是使用三種欄位值來表示，分別為如下，主要會依據著declaration所在的selector 是用什麼形式來表示
	...
	- id 表示目前declaration 所在的 selector 有使用 id 來描述 selector 的具體程度，會用數字表示：
		- 若指向元件與1個id selector的描述相符，就+1-0-0，
		- 數字越高就表示在以用特定id形式描述該元件的具體程度就越高
	- class 表示目前declaration 所在的 selector 有使用 class 來描述 selector 的具體程度，會用數字表示，若指向元件與class selector描述數字越高就表示在以用特定class形式描述該元件的具體程度就越高
...
#🧠 在CSS Specificity中，每一個declaration權重形式是三種欄位值，分別為id、class、type，形式為id-class-type，那麼id代表著什麼？(請盡量描述增量) ->->-> ` id 表示目前declaration 所在的 selector 有使用 id 來描述 selector 的具體程度，會用數字表示，若特定元件與id selector的描述相符，就+1-0-0，數字越高就表示在以用特定id形式描述該元件的具體程度就越高`
