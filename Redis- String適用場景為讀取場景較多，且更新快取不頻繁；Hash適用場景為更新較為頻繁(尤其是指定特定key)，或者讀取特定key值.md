...
#🧠 Redis 上 string 和hash分別如何處理修改資料上的部分資料 ->->-> `以String來說的話，要對部分資料進行修改，就必須為了那個值，而將整筆資料(包含未要修改的屬性值)都要一起改；反之，以Hash來說的話，要修改特定屬性的值，由於會將資料切分成好幾分獨立的部分來儲存，每個又是以key-value pair來表示，所以只要指定對應key就能直接修改`

#🧠 Redis 上 string 和hash分別如何處理新增資料上的部分資料 ->->-> `以String來說的話，要新增新資料成為特定資料上的部分資料，就必須為了那個值，而將新增前的特定資料(包含未要修改的屬性值)都要一起改；要修改特定屬性的值，由於會將資料切分成好幾分獨立的部分來儲存，每個又是以key-value pair來表示，所以只要指定對應key就能直接修改`

#🧠 Redis 上 string 和hash分別如何處理新增/讀取/修改/刪除資料上的部分資料 ->->-> `以String來說的話，要對部分資料進行修改，就必須為了那個值，而整筆資料(包含未要修改的屬性值)都要一起改；要修改特定屬性的值，由於會將資料切分成好幾分獨立的部分來儲存，每個又是以key-value pair來表示，所以只要指定對應key就能直接修改`


#🧠 Redis 上 string 和 hash 分別如何處理新增/讀取/修改/刪除全部資料：string vs. hash ->->-> `以String來說，就直接更改value值，不需要指定key來進行操作；以Hash來說，就必須指定每個key來進行操作`
