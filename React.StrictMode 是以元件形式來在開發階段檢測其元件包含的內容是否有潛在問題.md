...
#🧠 React.StrictMode 是用來作檢測開發時期是否有潛在問題的元件，具體是是什麼？ ->->-> `React.StrictMode會是以元件來表示，並只針對它所包含的內容來進行檢測`

#🧠 React：若在開發階段發現原本只執行一次的操作，但額外被系統重複執行N次，最主要會是有什麼可能 ->->-> ``

#🧠 在React.StrictMode所包裹著的元件中，它會多執行1~N次生命週期會遇到的階段函式來檢測，是為了什麼？ ->->-> `會藉由重複執行來比對結果來判斷`
