...
在這裡按鈕綁定點擊事件的事件處理clickHandler，並且預期是透過點擊事件來變更title的內容，但結果毫無反應



這是因為
1. title 是有變更了，只是沒重新渲染成title內容被變更後的畫面
2. 預設上只有使用對應元件的標籤，才會渲染一次，接著就不會重複渲染，但這發生在渲染之後，所以也就沒渲染
...
解法：
1. 告知系統綁定要改變內容的實際DOM節點
2. 透過事件處理來擷取該節點，並更改其內容


### 元件上的對應函式之初始呼叫

當以這形式來表示ExpenseItem元件時，就是呼叫ExpenseItem元件對應的函式，並執行函式內容和會回傳
